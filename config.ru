...
questions1 =
[ qa('Objects, classes, and everything are really just fancy syntax on top of what every-day data structure?', 'Hashes'),
  qa('Their keys are what type of object?', 'symbols'),
  qa('An object is a hash with what keys?', ':class, and :instance_variables'),
  qa('Are instance_variables data or instructions?', 'data'),
  qa('A class is a hash with what keys?', ':superclass, :methods, :constants, :class, :instance_variables'),
  qa('Are methods data or instructions?', 'instructions'),
  qa('A binding is a hash with what keys?', ':self, :local_variables, :return_value, :next_binding'),
  qa('Are local variables data or instructions?', 'data'),
  qa('When do local variables come into existence?', 'When the binding is put on the stack'),
  qa('When do local variables die?', 'When the binding is removed from the stack (eg when we leave the method)'),
]

questions2 = [
  qa('Any time you have a name for something, you can represent this with what?', 'A hash'),
  qa('If I said that people have names and ages, what might it look like if we represent this as a hash?', '{name: "Josh", age: 32}'),
  qa('If I said that linked lists have a head, which is a node, and nodes have data and a link to the next node, what might our hash look like?', '{ head: { data: "hello", link: { data:"world", link: { data: "!", link: nil } } } }'),
  qa('If the object doesn\'t have methods, how does it manipulate its data?', 'by looking up its class'),
  qa('What about a class\'s structure makes it an object?', 'Its :class and :instance_variables'),
  qa('If we don\'t find the method we\'re looking for in the class, where do we go next?', 'the superclass'),
  qa('We say classes are also instances.... what does this mean?', 'it means they have instance variables and a class pointer'),
...
questions3 = [
  qa('Instance variables are stored where?', 'in objects'),
  qa('Methods are stored where?', 'in classes'),
  qa('Objects find their methods by checking which key in their hash?', ':class'),
  qa('Classes find their inherited methods by checking which key in their hash?', ':superclass'),
  qa('Constants are stored where?', 'in classes'),
  qa('local variables are stored where?', 'in bindings'),
  qa('If you know what a linked list is: how is the stack a linked list?', 'Each binding has some data we care about, and a link to the next one'),
  qa('If you know what a linked list is: how is inheritance a linked list?', 'The object is the base of the list, the head of the list is its class, the link to the next node is the superclass, the methods are the data'),
]

run lambda { |env|
  case env['PATH_INFO']
  when '/3' then html = html_for(questions3)
  when '/2' then html = html_for(questions2)
  else           html = html_for(questions1)
  end
